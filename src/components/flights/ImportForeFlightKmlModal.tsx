...
import { uploadFlightTrackFile, upsertFlight } from '@/lib/api/flight'
...
function dateISOInChicago(iso: string) {
  ...
  return new Intl.DateTimeFormat('en-CA', { timeZone: CHICAGO_TZ, year: 'numeric', month: '2-digit', day: '2-digit' }).format(d)
}
...
export default function ImportForeFlightKmlModal({
  open,
  parsed,
  onClose,
}: {
  open: boolean
  parsed: ParsedKml | null
  onClose: () => void
}) {
  ...
  const allFlights = useMemo(() => state.flightIds.map((id) => state.flightsById[id]), [state])
  ...
  async function attachToFlight(flight: Flight) {
    ...
    try {
      ...
      const saved = await uploadFlightTrackFile(updated.id, 'FORE_FLIGHT', p.file)
      const feature = saved.feature
      feature.properties = { ...(feature.properties ?? {}), id: updated.id }

      dispatch({ type: 'UPSERT_FLIGHT', flight: { ...updated, trackMeta: saved.meta ?? null, trackSource: saved.source } })
  ...
  async function createAndAttach() {
    ...
    try {
      ...
      const id = exists ? `${baseId}-${Math.random().toString(36).slice(2, 6)}` : baseId
      ...
      const saved = await uploadFlightTrackFile(flight.id, 'FORE_FLIGHT', p.file)
      const feature = saved.feature
      feature.properties = { ...(feature.properties ?? {}), id: flight.id }

      dispatch({ type: 'UPSERT_FLIGHT', flight: { ...flight, trackMeta: saved.meta ?? null, trackSource: saved.source } })
    ...
    <div className="modalOverlay" role="dialog" aria-modal="true">
      <div className="modalCard">
        <div className="modalHeader">
          <div>
            <div style={{ fontWeight: 900, fontSize: 16 }}>Import ForeFlight KML</div>
            <div className="muted" style={{ marginTop: 4 }}>
              {p.originalFilename} · {fmtChicago(p.startTimeISO)} → {fmtChicago(p.endTimeISO)} · ~{(durationMin / 60).toFixed(1)} hrs
            </div>
          </div>
        ...
        <div className="modalBody">
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <div className="muted">Candidates are matched by time overlap (±{BUFFER_MINUTES} min)</div>
          </div>

          {mode === 'existing' ? (
            <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr', gap: 10 }}>
              <div className="card" style={{ margin: 0 }}>
                <div className="cardTitle">Candidates</div>
                {candidates.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {candidates.slice(0, 8).map((f) => (
                      <button
                        key={f.id}
                        className={selectedExistingId === f.id ? 'mapListItem active' : 'mapListItem'}
                      ...
                      >
                        ...
                        <div className="muted">
                          ...
                          {f.startTimeISO && f.endTimeISO ? ` · ${fmtChicago(f.startTimeISO)} → ${fmtChicago(f.endTimeISO)}` : ''}
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div className="card" style={{ margin: 0 }}>
                <div className="cardTitle">Pick any flight</div>
                <select className="select" value={selectedExistingId} onChange={(e) => setSelectedExistingId(e.target.value)}>
                ...
                <div className="muted" style={{ marginTop: 8 }}>This sets the flight start/end time to the KML range.</div>
              </div>
            </div>
          ) : (
            <div className="card" style={{ marginTop: 14 }}>
              <div className="cardTitle">New flight details</div>
              <div className="grid2">
                <div>
                  <div className="muted">Tail</div>
                  <input className="input" value={newTail} onChange={(e) => setNewTail(e.target.value)} placeholder="N77GX" />
                </div>
                <div>
                  <div className="muted">Date</div>
                  <input className="input" value={dateISOInChicago(p.startTimeISO)} disabled />
                </div>
                <div>
                  <div className="muted">From</div>
                  <input className="input" value={newFrom} onChange={(e) => setNewFrom(e.target.value)} placeholder="KPAO" />
                </div>
                <div>
                  <div className="muted">To</div>
                  <input className="input" value={newTo} onChange={(e) => setNewTo(e.target.value)} placeholder="KMRY" />
                </div>
              </div>
              <div style={{ height: 10 }} />
              ...
              <input className="input" value={newTags} onChange={(e) => setNewTags(e.target.value)} placeholder="Training, Dual" />
              <div style={{ height: 12 }} />
              <button className="btnPrimary" disabled={saving} onClick={createAndAttach}>

...
export async function readForeFlightKmlTrack(
  file: File,
): Promise<ForeFlightKmlTrack> {
  ...
  const whens = Array.from(track.getElementsByTagNameNS(KML_NS, 'when'))
    .map((n) => n.textContent?.trim())
    .filter(Boolean) as string[];
  const coords = Array.from(track.getElementsByTagNameNS(GX_NS, 'coord'))
    .map((n) => n.textContent?.trim())
    .filter(Boolean) as string[];
  ...
  if (coordinates.length < 2)
    throw new Error('No valid coordinates found in this KML file');
  ...
  for (let i = 1; i < samplesFull.length; i++) {
    ...
    const dtSec =
      (new Date(cur.t).getTime() - new Date(prev.t).getTime()) / 1000;
  ...
  const altVals = samples
    .map((s) => s.altAglFt)
    .filter((x): x is number => typeof x === 'number' && Number.isFinite(x));
  const gsVals = samples
    .map((s) => s.gsKt)
    .filter((x): x is number => typeof x === 'number' && Number.isFinite(x));
  ...
  const gsAvgKt = gsVals.length
    ? gsVals.reduce((a, b) => a + b, 0) / gsVals.length
    : null;
